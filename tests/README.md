...
```bash
sudo cp config.yaml.example /etc/hetzner/failover-config.yaml
sudo chmod 600 /etc/hetzner/failover-config.yaml
```

3. Edit the configuration file with your settings:
```bash
sudo vi /etc/hetzner/failover-config.yaml
```

## Configuration

Edit `/etc/hetzner/failover-config.yaml`:
...
```bash
configure
set high-availability vrrp group VRRP01 transition-script master '/usr/local/bin/hetzner-vrrp-failover -c /etc/hetzner/failover-config.yaml'
commit
save
```
...
```bash
# Execute failover
/usr/local/bin/hetzner-vrrp-failover -c /etc/hetzner/failover-config.yaml

# Dry run (validate config without making changes)
/usr/local/bin/hetzner-vrrp-failover -c /etc/hetzner/failover-config.yaml --dry-run
```
